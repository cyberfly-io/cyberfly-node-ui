...
import { Button, Card, CardContent, Chip, Container, Grid, Stack, TextField, Typography, Alert, Snackbar, Switch, FormControlLabel, LinearProgress, IconButton, Box } from '@mui/material'
...
const StreamPage = () => {
		// dark mode currently not used directly but kept for future theming adjustments
		useDarkMode()
	...
	const hexPreview = (buf) => {
		if (!buf || buf.length < 8) return 'N/A'
		const u8 = buf instanceof Uint8Array ? buf : new Uint8Array(buf)
		return Array.from(u8.slice(0, 8)).map(b => b.toString(16).padStart(2, '0')).join(' ')
	}
	...
	// keep debugLogRef in sync with state toggle
...
// (legacy duplicate requestInit removed; using useCallback version defined earlier)
	...
	const startBroadcast = async () => {
		...
		if (!lp) return
		try {
			if (debugLogRef.current) console.log('[STREAM][DEBUG] startBroadcast invoked')
			...
			if (bootstrapAddr) {
				...
				let connectedIds = lp.getConnections().map(c => c.remotePeer.toString())
				if (bootstrapPeerId && !connectedIds.includes(bootstrapPeerId)) {
					try {
						...
						connectedIds = lp.getConnections().map(c => c.remotePeer.toString())
					} catch (e) {
			...
			if (!selfPeerId) {
				try { setSelfPeerId(lp.peerId.toString()) } catch {}
			}
			...
			if (sourceMode === 'camera') {
				...
				try {
					...
					if (!mediaStream) throw new Error('captureStream not supported for this file')
				} catch (e) {
				...
				try {
					...
					if (!videoStream) throw new Error('captureStream not supported for this file')
			...
			let selected = preferred.find(m => MediaRecorder.isTypeSupported(m)) || ''
			const recorderOpts = selected ? { mimeType: selected, videoBitsPerSecond: 600_000 } : {}
			...
			try {
				if (!lp.services.pubsub.getTopics().includes(topicName)) {
					console.log('[STREAM][BROADCAST] self-subscribing to topic before publish')
					// Some pubsub impls allow handler arg; we just call bare subscribe here
					await lp.services.pubsub.subscribe(topicName)
				}
			} catch (e) { console.warn('[STREAM][BROADCAST] self subscribe failed (non fatal)', e) }
			...
			if (debugLogRef.current) console.log('[STREAM][SEND INIT CONFIRM] topics now', lp.services.pubsub.getTopics?.())
				...
				try {
					...
					if (parsed.type === 'request-init') {
						...
						if (initSegmentRef.current) {
							...
							lp.services.pubsub.publish(topicName, new TextEncoder().encode(JSON.stringify({ ...initPayload, ts: Date.now(), replay: true }))).catch(()=>{})
						}
					}
				} catch {}
			}
			...
			mr.ondataavailable = async (e) => {
				...
				if (nextSeq === 0 && !initSegmentRef.current) {
					try { initSegmentRef.current = frame.slice() } catch (e) { initSegmentRef.current = new Uint8Array(frame) }
					// Proactively publish init WITH b64 to help early viewers who already requested init before first segment was ready
					try {
						const b64 = bufToBase64(initSegmentRef.current)
						const enriched = { type: 'init', mimeType: selected || mr.mimeType, ts: Date.now(), from: lp.peerId.toString(), replay: true, b64, firstSegment: true }
						await lp.services.pubsub.publish(topicName, new TextEncoder().encode(JSON.stringify(enriched)))
						if (debugLogRef.current) console.log('[STREAM][SEND INIT WITH B64] first segment bytes=', initSegmentRef.current.length)
					} catch (ePublish) { console.warn('[STREAM][SEND INIT WITH B64 ERROR]', ePublish) }
				}
				if (debugLogRef.current) console.log('[STREAM][SEND CHUNK PROTO] seq', nextSeq, 'bytes', frame.length)
				...
				if (!protocolSent) { // send via pubsub until at least one protocol consumer is connected
					try {
						...
						if (debugLogRef.current) console.log('[STREAM][SEND CHUNK PUBSUB FALLBACK] seq', nextSeq)
					} catch (eFallback) { console.warn('[STREAM][PUBSUB FALLBACK ERROR]', eFallback) }
				}
			...
			reinitIntervalRef.current = setInterval(() => {
				...
				lp.services.pubsub.publish(topicName, new TextEncoder().encode(JSON.stringify(rein))).then(()=>{
					if (debugLogRef.current) console.log('[STREAM][RE-SEND INIT]', rein)
			...
			try {
				lp.handle(VIDEO_PROTOCOL, async ({ stream, connection }) => {
					if (debugLogRef.current) console.log('[STREAM][PROTO] viewer connected', connection.remotePeer.toString())
					// create a pushable queue for this consumer
					class AsyncQueue {
						constructor() { this.items = []; this.resolvers = []; this.ended = false }
						push(item) { if (this.ended) return; if (this.resolvers.length) { const r = this.resolvers.shift(); r({ value: item, done: false }) } else this.items.push(item) }
						close() { this.ended = true; while (this.resolvers.length) { const r = this.resolvers.shift(); r({ done: true }) } }
						[Symbol.asyncIterator]() { const self = this; return { next() { if (self.items.length) return Promise.resolve({ value: self.items.shift(), done: false }); if (self.ended) return Promise.resolve({ done: true }); return new Promise(res => self.resolvers.push(res)) } } }
					}
					const q = new AsyncQueue()
					const writerPromise = stream.sink(q[Symbol.asyncIterator]())
					const consumerObj = { id: connection.remotePeer.toString(), push: (c) => q.push(c), end: () => q.close() }
					...
					if (initSegmentRef.current) {
						try {
							...
							if (debugLogRef.current) console.log('[STREAM][PROTO] sent init segment to', consumerObj.id, 'bytes=', initBuf.length)
						} catch (e) { console.warn('[STREAM][PROTO] init send failed', e) }
					}
					// also drain any inbound data (viewer->broadcaster) but ignore content
					(async () => {
						try {
							for await (const _ of stream.source) { void _ }
						} catch (e) { /* ignore */ }
					})()
					// wait for writer finish then cleanup
					writerPromise.then(() => {
						consumersRef.current = consumersRef.current.filter(c => c.id !== consumerObj.id)
						consumerObj.end()
						if (debugLogRef.current) console.log('[STREAM][PROTO] writer finished for', consumerObj.id)
					}).catch(() => {
						consumersRef.current = consumersRef.current.filter(c => c.id !== consumerObj.id)
						consumerObj.end()
					})
	...
	// Cleanup on unmount
		useEffect(() => {
			return () => {
				...
				stopBroadcast()
				unsubscribe()
				connIntervalRef.current && clearInterval(connIntervalRef.current)
			}
			// not adding deps to avoid re-running on state changes; functions stable enough for unmount cleanup
			// eslint-disable-next-line react-hooks/exhaustive-deps
		}, [])
		...
		const connectToManualPeer = async () => {
			...
			if (!target) { setManualConnectMsg('Enter peer id'); return }
			const lp = await ensureLibp2p()
			if (!lp) { setManualConnectMsg('Libp2p not ready'); return }
			...
			if (!success) setManualConnectMsg(prev => prev + ' | All candidates failed')
		}

	return (
		<Container maxWidth="xl" sx={{ py: { xs: 2, md: 3 } }}>
			<Stack spacing={3}>
				<Typography variant="h4" fontWeight={700}>P2P Video Stream</Typography>
				<Grid container spacing={3}>
					<Grid item xs={12} md={4}>
						<Card elevation={2} sx={{ height: '100%' }}>
							<CardContent>
								<Stack spacing={2}>
									<Typography variant="h6" fontWeight={600}>Channel Control</Typography>
									{/* File picker: choose a video file to broadcast instead of camera */}
									<Box sx={{ mt: 1, mb: 1 }}>
										<Typography variant="caption" color="text.secondary">Source</Typography>
										<Stack direction="row" spacing={1} sx={{ mt: 0.5 }} alignItems="center">
											<Stack direction="row" spacing={1} sx={{ mr: 1 }}>
												<Button size="small" variant={sourceMode === 'camera' ? 'contained' : 'outlined'} onClick={() => setSourceMode('camera')}>Camera</Button>
												<Button size="small" variant={sourceMode === 'file' ? 'contained' : 'outlined'} onClick={() => setSourceMode('file')}>File</Button>
												<Button size="small" variant={sourceMode === 'file-audio' ? 'contained' : 'outlined'} onClick={() => setSourceMode('file-audio')}>File + Camera Audio</Button>
											</Stack>
											<input
												type="file"
												accept="video/*"
									...
									{selfPeerId && (
										<Box>
											...
											<Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 0.5 }}>
												<Typography component="span" sx={{ fontFamily: 'monospace', fontSize: '0.65rem', wordBreak: 'break-all', bgcolor: 'action.hover', px: 1, py: 0.5, borderRadius: 1, flex: 1 }}>
													{selfPeerId}
												</Typography>
										...
										<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
											...
											{!libp2pState && (
												<Button
													size="small"
													variant="outlined"
													onClick={ensureLibp2p}
													disabled={libp2pInitializing}
												>
									...
									<FormControlLabel
										control={<Switch checked={autoForward} onChange={e => setAutoForward(e.target.checked)} />}
										label="Auto Forward (assist distribution)"
									/>
									...
									<Box sx={{ mt: 1, mb: 1 }}>
										...
										<Stack direction="row" spacing={1} sx={{ mt: 0.5 }} alignItems="center">
											...
											<FormControlLabel
												control={<Switch size="small" checked={useWebRTC} onChange={e => setUseWebRTC(e.target.checked)} />}
									...
									<Stack direction="row" spacing={1} flexWrap="wrap">
										...
										<Chip label={broadcasting ? 'Broadcasting' : subscribed ? 'Viewing' : 'Idle'} color={broadcasting ? 'success' : subscribed ? 'info' : 'default'} />
										...
										<Chip label={connectionType === 'webrtc' ? 'WebRTC Mode' : 'Protocol Mode'} variant="outlined" />
										{subscribed && (
											<Chip 
										...
										{subscribed && streamHealth.bufferHealth !== 'unknown' && (
											<Chip 
												...
												color={streamHealth.bufferHealth === 'healthy' ? 'success' : streamHealth.bufferHealth === 'low' ? 'warning' : 'error'}
												variant="outlined"
											/>
										)}
									</Stack>
													<FormControlLabel sx={{ mt: -1 }} control={<Switch size="small" checked={debugLog} onChange={e => setDebugLog(e.target.checked)} />} label={<Typography variant="caption">Debug Logging</Typography>} />
													
													{/* Stream Recovery Controls */}
													{subscribed && (
														<Box sx={{ mt: 1 }}>
															<Stack direction="row" spacing={1} alignItems="center">
																<Button 
																	...
																	onClick={() => {
																		console.log('[STREAM][MANUAL RECOVERY] User triggered recovery')
																		// Reset MediaSource pipeline
																		resetMediaSourcePipeline()
																		// Request fresh init
																		try {
																			const lp = libp2pState
																			if (lp) {
																				lp.services.pubsub.publish(topicName, new TextEncoder().encode(JSON.stringify({
																					type: 'request-init',
																					ts: Date.now(),
																					from: lp.peerId.toString(),
																			...
																			console.warn('[STREAM][MANUAL RECOVERY] Failed to request init', e)
																		}
																	}}
																>
																...
																{streamHealth.isStalled && (
																	<Typography variant="caption" color="error">
																		Stream appears stalled - no frames for {Math.floor((Date.now() - lastFrameTimeRef.current) / 1000)}s
																	</Typography>
																)}
															</Stack>
														</Box>
													...
													<Button size="small" variant="outlined" onClick={() => {
														try {
															...
															console.log('[STREAM][DEBUG BUTTON] topics =>', topics)
															if (lp.services.pubsub.getSubscribers) {
																const subs = lp.services.pubsub.getSubscribers(topicName) || []
																console.log('[STREAM][DEBUG BUTTON] subscribers for', topicName, '=>', [...subs].map(p=>p.toString?.()||p))
															}
														} catch (e) { console.warn('Topic debug failed', e) }
													}}>Topics/Subs</Button>
													<Stack spacing={1}>
														<Typography variant="subtitle2">Manual Peer Connect</Typography>
														<TextField size="small" placeholder="Peer ID" value={manualPeerId} onChange={e => setManualPeerId(e.target.value)} />
														<Stack direction="row" spacing={1}>
															<Button size="small" variant="contained" onClick={connectToManualPeer}>Connect</Button>
															{manualConnectMsg && <Typography variant="caption" sx={{ alignSelf: 'center' }}>{manualConnectMsg}</Typography>}
														</Stack>
														{connectionsSnapshot.length > 0 && (
															<Box sx={{ maxHeight: 100, overflowY: 'auto', border: '1px dashed', borderColor: 'divider', p: 1, borderRadius: 1 }}>
																{connectionsSnapshot.map(c => (
																	<Typography key={c.id} variant="caption" sx={{ display: 'block', fontFamily: 'monospace' }}>{c.id} • streams:{c.streams} • {c.status}</Typography>
																))}
															</Box>
														)}
														
														{/* WebRTC Connections Display */}
														{webrtcConnections.length > 0 && (
															<Box sx={{ mt: 1 }}>
																<Typography variant="caption" color="text.secondary">WebRTC Connections</Typography>
																<Box sx={{ mt: 0.5, maxHeight: 100, overflowY: 'auto', p: 1, border: '1px solid', borderColor: 'success.main', borderRadius: 1, bgcolor: 'success.light', opacity: 0.9 }}>
																	{webrtcConnections.map(peerId => (
																		<Stack key={peerId} direction="row" spacing={1} alignItems="center" sx={{ mb: 0.5 }}>
																			<Typography component="span" sx={{ fontFamily: 'monospace', fontSize: '0.6rem', wordBreak: 'break-all', flex: 1, color: 'success.dark' }}>
																				{peerId} • WebRTC
																			</Typography>
																			<Button
																				size="small"
									...
									{participants.length > 0 && (
										<Box>
											...
											<Box sx={{ mt: 0.5, maxHeight: 140, overflowY: 'auto', p: 1, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
												{participants.map(pid => (
													<Stack key={pid} direction="row" spacing={1} alignItems="center" sx={{ mb: 0.5 }}>
														<Typography component="span" sx={{ fontFamily: 'monospace', fontSize: '0.6rem', wordBreak: 'break-all', flex: 1 }}>{pid}</Typography>
									...
									{broadcasting && lastChunkTs && (
										<Typography variant="caption" color="text.secondary">Last chunk: {lastChunkTs.toLocaleTimeString()}</Typography>
									)}
									{initializing && <LinearProgress />}
									<Alert severity="info" variant="outlined" sx={{ fontSize: '0.75rem' }}>
										Experimental peer-assisted live streaming using libp2p pubsub. Short segments are relayed through the gossip network. Performance depends on codec support and network conditions.
									</Alert>
								</Stack>
							</CardContent>
						</Card>
						...
						<Grid container spacing={2}>
							<Grid item xs={12} md={6}>
								<Card elevation={2}>
									<CardContent>
										<Stack spacing={1}>
											<Typography variant="subtitle1" fontWeight={600}>Local</Typography>
											<video
												ref={videoLocalRef}
												autoPlay
												muted
									...
									<CardContent>
										<Stack spacing={1}>
											<Stack direction="row" justifyContent="space-between" alignItems="center">
												<Typography variant="subtitle1" fontWeight={600}>Remote</Typography>
			...
			>
				<Alert severity={snackbar.severity} onClose={closeMsg}>{snackbar.message}</Alert>
			</Snackbar>
		</Container>
	)
}
